...
use actix_web::{App, HttpServer, dev::ServiceRequest, Error as ActixError, error::ErrorUnauthorized};
...
async fn main() -> std::io::Result<()> {
    ...
    println!("Starting nginx_wrapper_api server...");
    println!("  Address: http://{}", addr);
    println!("  Workers: {}", workers);
    

    match env::var("API_KEY") {
        Ok(key) if !key.is_empty() => println!("  Authentication: Enabled"),
        _ => println!("  Authentication: Disabled (API_KEY not set)"),
    }
